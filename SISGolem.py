...
disp_arr = [6, 9, 20, 0, 20, 0, 10, 0, 0, 0]
seconds_between_requests = 20s
...
def get_classes(session, auth, course_subject='', catalog_number='', title_keyword=''):
    try:
...
def parse_item(item):
    '''
    Items come in formatted as
    [Name - Title, Catalog_number, Session, Days & Times, Room(Capacity), Instructor, Dates, Enrollment Cap, Enrolled, Fee, Select Class]
...
def output(classes, disp_arr):
    width =  sum(disp_arr) + 3*len([x for x in disp_arr if x]) - 1
    print('-'*width)
    if classes:
        open_classes = [x for x in classes if x[0] == "Open"]
        format_line(['Status', 'Name', 'Title', 'Cat #', 'Times', 'Room', 'Instructor', 'Dates', 'Enr Cap', 'Enr Tot'], disp_arr)
        print('-'*width)
        try:
            for line in classes:
                format_line(line, disp_arr)
        except Exception as e:
            log(str(e))
...
def format_line(line, disp_arr):
    for i in range(len(line)):
        if disp_arr[i] != 0:
           if disp_arr[i] < len(line[i]):
               print(line[i][0:disp_arr[i]], end=' | ')
           else:
               print(line[i].ljust(disp_arr[i]), end=' | ')
    print()
...
def check_classes(session, auth, class_list):
    ...
    for subject in subjects:
        ...
        classes = get_classes(session, authentication, course_subject=subject)
        if not classes:
            log('No results')
        else:
            for cls in classes:
...
def main():
    ...
    write_csv('temp/EECS_classes.csv', classes)
    output(classes, disp_arr)
